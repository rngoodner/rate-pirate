...
Flight data comes from **scraping Google Flights with headless Chrome** (`server/src/providers/google-flights.ts`) — prices are parsed from result aria-labels, one throttled page load per route-month. Do not suggest Amadeus (self-service portal decommissioned July 2026) or rely on Travelpayouts (measured ~2% ABQ-origin coverage; its client remains only as a fallback).
...
Copy `.env.example` to `.env` for local config (loaded from the repo root regardless of workspace cwd). `PROVIDER=google-flights|travelpayouts|mock`; defaults to mock when unset without a Travelpayouts token. Live provider smoke test: `npm -w server exec tsx scripts/gf-smoke.ts` (a few real page loads).
...
- `providers/` — `FlightPriceProvider` seam (`monthQuotes` per route+month); `google-flights.ts` primary (puppeteer-core), `travelpayouts.ts` fallback, `mock.ts` fixtures + seeded synthetic prices. All automated tests use the mock.
