...
- **Dormant-pair skipping** (`scan_state` table): a route-month-cabin that returns zero fares on 5 consecutive scans (e.g. no business/first cabin on a leisure route) is put to sleep and skipped, then re-probed every 14 days so newly-added or seasonal service is picked back up. Without this an always-empty pair has no snapshot, so the planner would treat it as never-scanned and give it *top* priority forever — the fix reclaims that wasted budget for routes that actually have fares.
- **Shown-deal verification**: after each batch's planned scans, every currently-displayed deal (active, monitored cabin, in-horizon) whose route-month the batch *didn't* already scan is re-scraped. Fares still present → normal detection refreshes the price or expires it against the floor; **zero fares → the deal is expired outright** (the shown fare is gone). This zero-fare expiry lives in the shared per-route scan so the main loop gets it too. Active-deal counts are small, so the extra scrapes are cheap (budget-capped); it stops a vanished fare (the volatile-business-fare case) from lingering on the feed.
