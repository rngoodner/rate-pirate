...
describe('repo', () => {
  it('inserts and queries snapshots by route-month and route', () => {
    ...
    expect(snapshotsForRouteMonth(db, 'ABQ', 'NAP', '2026-08', 60)).toHaveLength(1);
    expect(snapshotsForRoute(db, 'ABQ', 'NAP', 90)).toHaveLength(2);
    const row = snapshotsForRouteMonth(db, 'ABQ', 'NAP', '2026-08', 60)[0]!;
  ...
  it('honors capturedAt overrides and an asOf window (simulator support)', () => {
    ...
    expect(snapshotsForRouteMonth(db, 'ABQ', 'NAP', '2026-08', 60, '2026-07-01 00:00:00')).toHaveLength(2);
    expect(snapshotsForRouteMonth(db, 'ABQ', 'NAP', '2026-08', 5, '2026-07-01 00:00:00')).toHaveLength(0);
    // asOf excludes future captures
    expect(snapshotsForRoute(db, 'ABQ', 'NAP', 60, '2026-05-11 00:00:00')).toHaveLength(1);
  });
  ...
  it('tracks latest capture per route-month for the planner', () => {
    ...
    const latest = latestCaptureByRouteMonth(db, 'ABQ');
  ...
  it('prunes old snapshots', () => {
    ...
    expect(snapshotsForRoute(db, 'ABQ', 'NAP', 365)).toHaveLength(1);
  });
